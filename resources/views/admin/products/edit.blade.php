...
@section('title', $title = 'ویرایش ' .$product->name)
                ...
                <form action="{{ route('products.update', $product->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="row with-forms">
                        <div class="col-md-12">
                            ...
                            <input type="text" class="search-field" name="name" value="{{ $product->name }}" placeholder="تسبیح دست ساز فانتزی" required autofocus>
                        </div>
                    </div>
                    <div class="row with-forms">
                        <div class="col-md-12">
                            ...
                            <input type="text" class="search-field" name="slug" value="{{ $product->slug }}" placeholder="تسبیح_دست_ساز_صورتی" required autofocus>
                        </div>
                    </div>
                    ...
                    <div class="row with-forms">
                        ...
                        <div class="col-md-6">
                            <h5>قیمت اصلی</h5>
                            <input type="text" name="price" value="{{ $product->price }}" placeholder="60000" required autofocus>
                        </div>
                        <div class="col-md-6">
                            <h5>قیمت فروش</h5>
                            <input type="text" name="sale_price" value="{{ $product->sale_price }}" placeholder="50000" required autofocus>
                        </div>

                        <!-- SKU -->
                        <div class="col-md-6">
                            <h5>SKU <i class="tip" data-tip-content="SKU شماره واحد محصول"><div class="tip-content"> SKU شماره واحد محصول</div></i></h5>
                            <input type="text" name="sku" value="{{ $product->sku }}" placeholder="MF-125" required autofocus>
                        </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <select class="chosen-select-no-single" name="status" style="display: none;">
                                @foreach($data['statuses'] as $key => $value)
                                    <option value="{{ $key }}" {{ $key == $product->status ? "selected" : "" }}>{{ $value }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="col-md-12">
                            <h5>توضیحات کوتاه</h5>
                            <textarea class="WYSIWYG" name="short_description" cols="40" rows="3" id="short_description" spellcheck="true" required>{{ $product->short_description }}</textarea>
                        </div>
                        <!-- Full Description -->
                        <div class="col-md-12">
                            <h5>توضیحات کامل</h5>
                            <textarea class="WYSIWYG" name="description" cols="40" rows="3" id="description" spellcheck="true" required>{{ $product->description }}</textarea>
                        </div>
                    </div>
